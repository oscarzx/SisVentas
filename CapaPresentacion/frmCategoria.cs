...
namespace CapaPresentacion
{
    public partial class frmCategoria : Form
    {
        ...
        {
            InitializeComponent();
            this.ttMensaje.SetToolTip(this.nombreTextBox, "Ingrese el nombre de la categoría");
        }
        ...
        private void guardarButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        rpta = NCategoria.Insertar(nombreTextBox.Text.Trim().ToUpper(), 
                            descripcionTextBox.Text.Trim());
                    }
                    else
                    {
                        rpta = NCategoria.Editar(Convert.ToInt32(idCategoriaTextBox.Text), 
